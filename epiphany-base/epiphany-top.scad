...
module top_hole (diameter, head_diameter, head_thickness) {
	translate (v = [	0, 0, -1])
		cylinder (h = BASE_THICKNESS + 2, r = diameter / 2, $fn = 24,
				    center = false);
}
...
module epiphany_top () {
	TOP_BASE_HEIGHT=BASE_THICKNESS*4;
	TOP_TOP_HEIGHT=2;
	difference() {
		cube (size = [BOARD_LENGTH, BOARD_WIDTH, TOP_BASE_HEIGHT]);
		translate (v = [PAD_OFF_1, PAD_OFF_1, 0])
			top_hole (BOARD_HOLE_DIAMETER, BASE_SCREW_HEAD_DIAMETER,
						 BASE_SCREW_HEAD_THICKNESS);
		translate (v = [PAD_OFF_1, BOARD_WIDTH - PAD_OFF_2, 0])
		 	top_hole (BOARD_HOLE_DIAMETER, BASE_SCREW_HEAD_DIAMETER,
						 BASE_SCREW_HEAD_THICKNESS);
		translate (v = [BOARD_LENGTH - PAD_OFF_2, BOARD_WIDTH - PAD_OFF_2, 0])
			top_hole (BOARD_HOLE_DIAMETER, BASE_SCREW_HEAD_DIAMETER,
						 BASE_SCREW_HEAD_THICKNESS);
		translate (v = [BOARD_LENGTH - PAD_OFF_2, PAD_OFF_1, 0])
			top_hole (BOARD_HOLE_DIAMETER, BASE_SCREW_HEAD_DIAMETER,
						 BASE_SCREW_HEAD_THICKNESS);
	   translate (v = [PAD_OFF_2*2, PAD_OFF_2*2, 0])
			cube (size = [BOARD_LENGTH-4*PAD_OFF_2, BOARD_WIDTH-4*PAD_OFF_2, TOP_BASE_HEIGHT]);
	}
	// Board holding half
	translate (v = [0, 0, TOP_BASE_HEIGHT])
	difference() {
		cube (size = [BOARD_LENGTH, BOARD_WIDTH, TOP_TOP_HEIGHT]);
	   translate (v = [PAD_OFF_2*1.5, PAD_OFF_2*1.6, 0])
			cube (size = [BOARD_LENGTH-3*PAD_OFF_2, BOARD_WIDTH-3*PAD_OFF_2, TOP_TOP_HEIGHT]);
	}
}
